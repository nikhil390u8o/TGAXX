...
async def deposit_init(c, m):
    ...
    # Step 1 of deposit: show QR → wait for UTR
    login_data[uid] = {"step": "dep_wait_utr", "ref": ref_id}
    ...
    await m.reply_photo(
        ...
        caption=(
            ...
            f"━━━━━━━━━━━━━━━\n"
            f"✅ **ᴀғᴛᴇʀ ᴘᴀʏɪɴɢ:**\n"
            f"**sᴇᴛᴘ 1 →** sᴇɴᴅ ʏᴏᴜʀ 12 ᴅɪɢɪᴛ ᴜᴛʀ ɴᴜᴍʙᴇʀ**"
        )
    )
...
async def handle_all_callbacks(c, q):
    ...
    # ── Admin approves deposit — ask how much to add ──
        ...
        parts = data.split("_")
        # format: aprv_pay_{uid}_{utr}_{amount}
        target_uid = int(parts[2])
        utr_val = parts[3]
        claimed_amt = parts[4] if len(parts) > 4 else "?"
        login_data[uid] = {
...
async def handle_inputs(c, m):
    ...
    # Step 1: UTR
    if state.get("step") == "dep_wait_utr":
        utr = text
        if not utr.isdigit() or len(utr) != 12:
            return await m.reply(
                "❌ **ɪɴᴠᴀʟɪᴅ ᴜᴛʀ!**\n\n"
                "ᴘʟs sᴇɴᴅ ʏᴏᴜʀ **12-ᴅɪɢɪᴛ** ᴜᴛʀ.\n"
                "ᴄʜᴇᴋ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ ᴛʀᴀɴsᴇᴄᴛɪᴏɴ ᴀɴᴅ ᴄᴏᴘʏ ᴜᴛʀ ᴀɴᴅ sᴇɴᴅ ᴍᴇ"
            )
        state["utr"] = utr
        state["step"] = "dep_wait_ss"
        await m.reply(
            f"✅ **ᴜᴛʀ sᴀᴠᴇᴅ:** `{utr}`\n\n"
            f"📸 **sᴇᴛᴘ 2: ɴᴏᴡ sᴇɴᴅ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ sᴄʀᴇᴇɴsʜᴏᴛ (photo):**"
        )
        return

    # Step 2: Screenshot
    elif state.get("step") == "dep_wait_ss":
        if not m.photo:
        ...
        await m.reply(
            "✅ **sᴄʀᴇᴇɴsʜᴏᴛ ʀᴇᴄɪᴠᴇᴅ!**\n\n"
            "💰 **sᴇᴛᴘ 3: ʜᴏᴡ ᴍᴜᴄʜ ᴅɪᴅ ʏᴏᴜ ᴘᴀʏ sᴇɴᴅ ᴛʜᴇ ᴀᴍᴏᴜɴᴛ ɪɴ ɴᴜᴍʙᴇʀs ʟɪᴋᴇ `10,20,30` ᴇᴛᴄ:**\n"
            "Example: `500`"
        )
        return

    # Step 3: Amount → send to admin
        ...
        kb = types.InlineKeyboardMarkup([
            [
                types.InlineKeyboardButton("✅ ᴀᴘᴘʀᴏᴠᴇ", callback_data=f"aprv_pay_{uid}_{utr}_{int(amt)}"),
        ...
        await bot.send_photo(
            ...
            caption=(
                ...
                f"💰 **ᴀᴍᴏᴜɴᴛ:** `₹{int(amt)}`\n"
                f"📝 **ʀᴇғ:** `{ref}`\n\n"
                f"👆 **ᴀᴘᴘᴛᴏᴠᴇ → ʏᴏᴜ ᴡɪʟʟ ʙᴇ ᴀsᴋᴇᴅ ʜᴏᴡ ᴍᴜᴄʜ ᴛᴏ ᴀᴅᴅ**"
            ),
            reply_markup=kb
        )
